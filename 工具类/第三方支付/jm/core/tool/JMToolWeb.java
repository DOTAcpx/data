...
/**
 * WEB工具类
 * @author 李劲 2012-9-3 下午7:39:43
 */
public abstract class JMToolWeb {

	private static Logger log = Logger.getLogger(JMToolWeb.class);
	...
	public static String getIpAddr(HttpServletRequest request) {
		String ip = request.getHeader("X-getRequest()ed-For");
		if (JMToolString.isEmpty(ip) || "unknown".equalsIgnoreCase(ip)) {
	...
	public static String getParam(HttpServletRequest request, String name) {
		...
		if (null != value && !value.isEmpty()) {
			try {
				return URLDecoder.decode(value, JMToolString.encoding).trim();
	...
	public static void outPage(HttpServletResponse response, String content, String contentType) {
		try {
			outPage(response, content.getBytes(JMToolString.encoding), contentType); // char to byte 性能提升
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void outPage(HttpServletResponse response, byte[] content, String contentType) {
		...
		response.setCharacterEncoding(JMToolString.encoding);
	...
	public static void outDownCsv(HttpServletResponse response, String content) {
		...
		try {
			response.setHeader("Content-Disposition", "attachment; filename=" + java.net.URLEncoder.encode(ToolDateTime.format(ToolDateTime.getDate(), ToolDateTime.pattern_ymd_hms_s) + ".csv", JMToolString.encoding));
		...
		// out.write(content);
		try {
			response.getOutputStream().write(content.getBytes(JMToolString.encoding));
		} catch (IOException e) {
			e.printStackTrace();
		}// char to byte 性能提升
			// out.flush();
	...
	public static String requestStream(HttpServletRequest request) {
		...
		try {
			request.setCharacterEncoding(JMToolString.encoding);
			inputStream = (ServletInputStream) request.getInputStream();
			inputStreamReader = new InputStreamReader(inputStream, JMToolString.encoding);
	...
	public static String getCookieValueByName(HttpServletRequest request, String name) {
		Map<String, Cookie> cookieMap = JMToolWeb.readCookieMap(request);
